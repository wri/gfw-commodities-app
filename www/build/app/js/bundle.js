...
define('map/LayerController',[
    ...
    return {
        ...
        updateHansenDates: function(clauseArray) {
          var hansenLayer = app.map.getLayer('hansenLoss');
        ...
        updateHansenTCD: function(layerConfig, densityRange) {
          ...
          var baseUrl = MapConfig.hansenLoss.url.split('/tc')[0];
          baseUrl += '/tc' + density;
          baseUrl += '/{z}/{x}/{y}.png';

          original30Layer.setUrl(baseUrl);
          original30Layer.setDateRange(minYear, maxYear);
...
define('layers/EsriTileCanvasBase',[
    'dojo/_base/declare',
    'esri/geometry/Point',
    'esri/layers/layer'
], function(declare, Point, Layer) {
  ...
  */
  return declare('EsriTileCanvasBase', [Layer], {
    ...
    */
    _setMap: function _setMap (map) {
      ...
      //- Add event listeners
      this._handles.push(this._map.on('extent-change', self._extentChanged.bind(this)));
      this._handles.push(this._map.on('zoom-start', self._onZoomStart.bind(this)));
      this._handles.push(this._map.on('pan-end', self._onPanEnd.bind(this)));
      this._handles.push(this._map.on('pan', self._onPan.bind(this)));
    ...
    */
    _fetchTile: function _fetchTile (tile) {
      ...
      this._fetchImage(url, function(image) {
        var canvas = document.createElement('canvas');
        canvas.height = self.options.tileSize;
    ...
    refresh: function refresh () {
      ...
      Object.keys(this.tiles).forEach(function(key) {
        ...
        if (tile.z > self.options.maxZoom) {
          var info = self._getSubrectangleInfo(tile);
          ctx.imageSmoothingEnabled = false;
          ctx.mozImageSmoothingEnabled = false;
...
define('components/LayerModal',[
  ...
  var LayerModal = React.createClass({displayName: "LayerModal",
    ...
    render: function() {
      ...
      return (
          React.createElement(ModalWrapper, {downloadData: downloadData}, 
...
define('utils/Loader',[
    ...
    return {
        ...
        getTemplate: function(name) {
            var deferred = new Deferred(),
                path = './app/templates/' + name + '.html?v=2.5.8',
                req;
