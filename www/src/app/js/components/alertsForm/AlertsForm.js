define([
	...
	'map/config',
	'map/MapModel'
], function(React, AlertsConfig, AnalyzerConfig, Units, Color, Graphic, esriRequest, Draw, Point, Extent, Circle, Polygon, scaleUtils, SimpleFillSymbol, SimpleLineSymbol, dom, dojoQuery, domClass, MapConfig, MapModel) {
	...
	AlertsForm = React.createClass({
		...
		componentDidMount: function() {
			...
			// Setup graphics references & update
			graphicsLayer = app.map.getLayer(MapConfig.customGraphicsLayer.id);
			customFeatures = graphicsLayer.graphics.slice(0);
			...
			return (
				React.DOM.div({className: 'alerts-form-root'},
					...
					React.DOM.div({className: 'alerts-form-header'},
						'Icon',
						'Alerts Registration',
						React.DOM.button({className: 'float-right'}, 'x')
					),
					// Body
					React.DOM.div({className: 'alerts-form-body'}, 
						...
						React.DOM.div({'className':'text-right'},
							React.DOM.button(null, 'Toggle All'),
							React.DOM.button({'className': 'clear-custom-features', 'onClick': this._clearFeatures}, 'clear all')
						),
						React.DOM.div({'className':'absolute no-wide', 'style': {border:'1px solid #DDDDDD', top: '360px', bottom:'131px'}},
		...
		_graphicsMapper: function(graphic) {
			return (
				React.DOM.div(null,
					React.DOM.input({type: 'checkbox'}),
					React.DOM.input({'value': graphic.attributes['WRI_label']})
				)
			)
		},

		_drawComplete: function(event) {
			console.debug('draw complete');
		},

		_toggleGraphic: function(event) {
			console.debug('toggle graphic')
		},
...
///////////////////
 ...
 //          'data-feature-id': item.attributes.WRI_ID,
 //          'onChange': this._graphicRename
 //        })
 //        )
 //    },

 //    _graphicRename: function(evt) {
 ...
 //          this.props.callback.updateAnalysisArea(graphic);
 //        }
 //      }
